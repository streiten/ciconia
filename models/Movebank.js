...
module.exports.getIndividualsEvents = (studyId,individualID,startts,endts,count) => {
  ...
  // var eventAttributes = "acceleration_axes";
  ...
  return request(options).then( body =>  {
        return new Promise( (resolve,reject) => {
          ...
          // ToDo: reject on non parsable body
           ...
           //   winston.log('info','Movebank fetched events:' + result.individuals[0].locations.length + ' for ' + result.individuals[0].individual_local_identifier);
           //  } else {
           //   winston.log('info','Movebank fetched no events for ' + individualID );
           // }
